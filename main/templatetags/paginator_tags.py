...
@register.simple_tag
def get_elided_page_range(instances, number, on_each_side=1, on_ends=1):
...
@register.inclusion_tag("web/pagination.html",takes_context=True)
def laod_pagination(context, instances):
    request = context['request']
    return {
        "instances":instances,
        "request":request
